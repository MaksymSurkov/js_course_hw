...
if (confirm('Почати тестування?')) {
	...
	document.write(`1. Загальну вартість (нові ціни - price) - ${totalPrice}<br>`);

	//* -------------------------------------------------- 2. Знайти кількість товарів, у яких ціна зменшилась (price < old_price).

	let discountedItemsCount = dataList.reduce(
		(prevItem, item) => item.price < item.old_price ? prevItem + 1 : prevItem, 0
	)

	document.write(`2. Знайти кількість товарів, у яких ціна зменшилась (price < old_price) - ${discountedItemsCount}<br>`);

	//* -------------------------------------------------- 3. Товари, які доступні (sell_status:"available").

	let availableItems = [];
	dataList.forEach(
		(item) => item.sell_status === "available" ? availableItems.push({ ...item }) : null
	)
	...
	dataList.forEach(
		(item) => {
			if (item.sell_status === "available") {
				...
				newAvailableItems.push({ id, price, old_price, usd_price });
			}
		}
	)
